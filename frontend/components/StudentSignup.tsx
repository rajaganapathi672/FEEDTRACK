...
export const StudentSignup: React.FC<StudentSignupProps> = ({ onBack, onSwitchToLogin, onSignup, loading, error }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onSignup({ name, email, studentId, password });
    };
    ...
    return (
        <div className="bg-background-dark text-white overflow-hidden selection:bg-white selection:text-black fixed inset-0 z-50">
            ...
            <main className="relative h-screen w-full flex">
                <section className="relative hidden md:flex flex-1 student-gradient-bg flex-col justify-center px-24 overflow-hidden border-r border-white/5">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="max-w-md text-white/40 font-sans text-sm leading-loose tracking-wide">
                            Create your student profile to begin sharing insights. Your academic credentials serve as the foundation for your secure, anonymous contribution to the institutional feedback loop.
                        </p>
                    </div>
                ...
                <section className="relative flex-1 bg-charcoal flex flex-col justify-center px-8 md:px-24 overflow-y-auto">
                    <div className="grain-overlay absolute inset-0"></div>
                    <div className="relative z-10 w-full max-w-sm mx-auto md:mx-0 py-20">
                        <div className="mb-12">
                            <h2 className="font-display text-4xl mb-2">Portal Alpha</h2>
                            <p className="font-mono text-[10px] uppercase tracking-widest text-white/40">Student Registration</p>
                        </div>
                        <form className="space-y-8" onSubmit={handleSubmit}>
                            ...
                            <div className="space-y-1">
                                <label className="font-mono text-[10px] uppercase tracking-widest text-white/30" htmlFor="email">Institutional Email</label>
                                <input
                                    className="input-line"
                                    id="email"
                        ...
                        <div className="mt-12">
                            <span
                                ...
                                className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/30 hover:text-white transition-colors cursor-pointer"
                            >
                                Already have an account? <span className="text-white ml-2 border-b border-white/20">Sign In</span>
                            </span>
                        </div>
                        <div className="mt-16 flex items-center gap-8 border-t border-white/5 pt-8">
                            <button className="flex items-center gap-2 group">
                                <span className="material-symbols-outlined text-sm text-white/20 group-hover:text-white transition-colors">shield</span>
                                <span className="font-mono text-[9px] uppercase tracking-widest text-white/20 group-hover:text-white transition-colors">GDPR Compliant</span>
                            </button>
                            <button className="flex items-center gap-2 group">
                                <span className="material-symbols-outlined text-sm text-white/20 group-hover:text-white transition-colors">help_center</span>
                                <span className="font-mono text-[9px] uppercase tracking-widest text-white/20 group-hover:text-white transition-colors">Support</span>
                            </button>
                        </div>
                    </div>
